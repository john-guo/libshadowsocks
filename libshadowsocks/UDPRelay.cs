...
namespace Shadowsocks.Relay
{
    class UDPRelay : Listener.Service
    {
        ...
        public void Close()
        {
            foreach (var item in this._cache.cacheMap)
            {
                item.Value.Value.value.Close();
            }
    ...
    class LRUCache<K, V> where V : UDPRelay.UDPHandler
    {
        ...
        internal Dictionary<K, LinkedListNode<LRUCacheItem<K, V>>> cacheMap = new Dictionary<K, LinkedListNode<LRUCacheItem<K, V>>>();
